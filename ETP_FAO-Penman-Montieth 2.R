## FAO-Penman-Montieth Evapotranspiration (ETP)
## By: Cesar Saavedra
## Junio, 2023
...
calc_ET0 <- function(yr, mn){
  outfile <- paste0(out_dir,'/FAO-Penman-Montieth/ET_0_monthly/ET-',yr,'-',mn,'.tif')
  # outfile2 <- paste0(out_dir,'/FAO-Penman-Montieth/ET_0_daily/',yr,'/ET-',yr,'-',mn,'-')
  cat(outfile, "\n")
  ...
  rm(p, cps, etmx, etmn, es, Dlt, ea, ET_0, tmx, tmn, sr, dp, tmean, ws_2m)
  gc(reset = T)
  cat("Done", "\n", ":)", "\n")
}
